...
import {
  ...
  Iimg,
  IUpdateProduct,
} from "../../redux/slice/product-Admin-redux/GetProAdm-Redux";
import {
  getProducts,
  updateProduct,
  changeAvailability,
  requestUpdateStatusProducts,
  clean,
  setProduct,
  updateAllPrices,
  cleanMsg,
...
const AdminManageProducts = () => {
  ...
  const {
    ...
    productEdit: productModal,
    productsUpdate: productsToUpdate,
    errorMessage: backMessage,
  } = useSelector((state: Ireducers) => state.reducerAdmin);
  ...
  const submitUpdateAllPrices = async (e: Event, modalForm: any) => {
    e.preventDefault();
    if (modalError.quantity || modalError.direction || modalError.type)
      return swal("Oops!", "Please fill all the fields correctly", "warning");
  ...
  const handleVisibility = (e:any, product:any) => {
    e.preventDefault();
    const { id } = product;
  ...
  const applyChanges = async () => {
    if (!productsToUpdate.length)
      return swal("Oops", "Please select product to change", "warning");
    dispatch(action.cleanProducts());
    await requestUpdateStatusProducts(productsToUpdate);

    swal(
      "Done",
      ` Products update: ${productsToUpdate
        .map((p:any) => p.name)
        .reduce((e:any, acc:any) => e + " & " + acc)}`
    );
    dispatch(clean());
    setActive(!active);
  };
  ...
  const handleOnFile = (event: any) => {
    ...
    if (formProduct.image.length >= 4)
      return swal("Oops!", "Only four images per product", "warning");
  ...
  if (!currentProducts[0])
    return (
      <div className={styles.products_manage__container}>
        <h1 className={styles.products_manage__title}>Loading...</h1>
      </div>
    );
      ...
      >
        ...
        >
          <div className={styles.modal__container__inside}>
            ...
            <p className={styles.current__data}>Choose only 4 images</p>
            <input
              type="file"
              accept=".jpg , .png , .jpeg"
