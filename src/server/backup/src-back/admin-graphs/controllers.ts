  ...
  await orders.forEach((order: any) => {
    ...
    if (years[year][months[month]][numberWeek]) {
      ...
      if (order.status === "confirmed")
        years[year][months[month]][numberWeek].confirmed++;
      if (order.status === "fulfilled")
        years[year][months[month]][numberWeek].fulfilled++;
      if (order.status === "in_process")
        years[year][months[month]][numberWeek].in_process++;
      if (order.status === "in_transit")
        years[year][months[month]][numberWeek].in_transit++;
      if (order.status === "canceled")
        years[year][months[month]][numberWeek].canceled++;
    } else {
...
export const productMVP = async (products: any, type: any) => {
  if (type) {
    const aux = await products
      .filter(
        (prod: any) => prod.evaluation.length > 0 && prod.category === type
      )
      .sort((a: any, b: any) => {
        return b.evaluation.length - a.evaluation.length;
      });

    const aux2: any = await aux
      .map((prod:any) => {
        let rating = 0;
    ...
    const aux = await products
      .filter((prod: any) => prod.evaluation.length > 0)
      .sort((a: any, b: any) => {
        return b.evaluation.length - a.evaluation.length;
      });
...
export const MSProducts = async (
  products: any,
  type: string | undefined | string[]
) => {
  if (type) {
    const productsWithOrders = await products
      .filter((prod: any) => prod.order.length > 0 && prod.category === type)
      .map((prod: any) => {
        let orders = 0;
    ...
    const productsWithOrders = await products
      .filter((prod: any) => prod.order.length > 0)
      .map((prod: any) => {
        let orders = 0;
        const aux = prod.order.forEach((ord: any) => {
          if (ord.status === "confirmed") orders = orders + 1;
